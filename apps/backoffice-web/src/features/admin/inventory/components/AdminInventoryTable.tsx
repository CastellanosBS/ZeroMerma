...
function stockTone(state: AdminInventoryStockState): "critical" | "neutral" | "success" | "warning" {
    ...
    <section className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[20px] border border-[var(--ui-color-border)] bg-white">
      <div className="flex min-w-0 shrink-0 items-center justify-between gap-3 border-b border-[var(--ui-color-border)] px-3 py-2.5">
        <div className="min-w-0">
          <h3 className="truncate text-base font-semibold text-slate-950" title="Inventario por sucursal">
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-y-auto p-2.5">
        {isLoading ? (
          <AdminEmptyState description="Consultando existencias reales del backend." title="Cargando inventario" />
        ) : errorMessage ? (
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span className="mt-0.5 block truncate text-xs text-slate-500">{item.className}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="mt-0.5 block truncate text-xs text-slate-500">{item.locationName}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="mt-0.5 block truncate text-xs text-slate-500">{item.unitOfMeasure}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="mt-0.5 block truncate text-xs text-slate-500">Ultimo movimiento</span>
                  </button>

                  <div className="flex min-w-0 flex-col gap-1.5 xl:items-end">
                    <div className="flex min-w-0 flex-wrap gap-1.5 xl:justify-end">
                      <StatusChip tone={stockTone(item.stockState)}>{formatStockState(item.stockState)}</StatusChip>
                      <StatusChip tone={warningTone(item.warningState)}>{warningLabel}</StatusChip>
                      {!item.productIsActive ? <StatusChip tone="warning">Producto inactivo</StatusChip> : null}
                    </div>
          ...
          <div className="grid gap-2">
            ...
            <p className="rounded-[16px] border border-[var(--ui-color-border)] bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600">
              Contrato activo: {backendContract.listEndpoint}. Las existencias aparecen cuando hay balances creados por
