...
function formatOwner(item: AdminPriceRow): string {
  return item.priceOwner === "product_unit_price" ? "Product unit_price" : "Class class_capture_unit_price";
}
  ...
  return (
    <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[20px] border border-[var(--ui-color-border)] bg-slate-50/80">
      <div className="shrink-0 border-b border-[var(--ui-color-border)] px-3 py-2.5">
        ...
        <h3 className="mt-1 truncate text-base font-semibold text-slate-950" title={item?.entityName ?? "Sin precio seleccionado"}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {isLoading ? (
          <article className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
            ...
            <p className="mt-1 text-sm leading-5 text-slate-600">Consultando propietario y salud comercial.</p>
          </article>
        ) : errorMessage ? (
          ...
          <div className="grid min-w-0 gap-3">
            <section className="min-w-0 rounded-[16px] border border-[var(--ui-color-border)] bg-white px-3">
              <Field label="Entidad" value={`${item.entityCode} - ${item.entityType === "product" ? "Producto" : "Clase"}`} />
            ...
            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              <div className="flex min-w-0 items-center justify-between gap-2">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Precio vigente</p>
                <button
            ...
            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Comparacion costo/precio</p>
              <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-[14px] bg-slate-100 px-3 py-2">
                  ...
                  <p className="font-semibold text-slate-950">{formatMoney(item.standardCost, item.currencyCode)}</p>
                </div>
                <div className="rounded-[14px] bg-slate-100 px-3 py-2">
            ...
            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              ...
              <div className="mt-2 grid gap-1.5 text-sm">
                ...
                <a
                  ...
                  href={item.relatedProductId ? `/admin/recetas-costos?product_id=${item.relatedProductId}` : "/admin/recetas-costos"}
                >
            ...
            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              ...
              <p className="mt-1 text-sm leading-5 text-slate-600">
                {detail?.historyNote ?? "El historial se consulta desde auditoria cuando este disponible."}
              </p>
            </section>
          </div>
        ) : (
          <article className="rounded-[16px] border border-dashed border-[var(--ui-color-border)] bg-white p-4">
            ...
            <p className="mt-1 text-sm leading-5 text-slate-600">
              Selecciona una fila para revisar propietario del precio, margen, advertencias y acciones.
            </p>
          </article>
        )}
      </div>
