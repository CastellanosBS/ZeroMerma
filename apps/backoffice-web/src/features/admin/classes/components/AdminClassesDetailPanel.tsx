  ...
  return (
    <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[20px] border border-[var(--ui-color-border)] bg-slate-50/80">
      <div className="shrink-0 border-b border-[var(--ui-color-border)] px-3 py-2.5">
        ...
        <h3 className="mt-1 truncate text-base font-semibold text-slate-950" title={classItem?.name ?? "Sin clase seleccionada"}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {isLoading ? (
          <article className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
            ...
            <p className="mt-1 text-sm leading-5 text-slate-600">Consultando la clase seleccionada.</p>
          </article>
        ) : errorMessage ? (
          ...
          <div className="grid min-w-0 gap-3">
            ...
            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              <div className="flex min-w-0 items-center justify-between gap-2">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Productos</p>
                <a
                ...
                <div className="mt-2 grid gap-1.5">
                  {classItem.linkedProducts.map((product) => (
                    <div
                      ...
                      key={product.id}
                    >
                      <span className="min-w-0 truncate font-semibold text-slate-800" title={product.name}>
                        {product.name}
                      </span>
                      <span className="shrink-0 text-slate-500">{product.status === "active" ? "Activo" : "Inactivo"}</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Salud operativa</p>
              <p className="mt-1 text-sm font-semibold text-slate-950">{formatReadiness(classItem.readiness)}</p>
              {classItem.warnings.messages.length > 0 ? (
                ...
                <p className="mt-1 text-sm leading-5 text-slate-600">Sin advertencias de configuracion.</p>
              )}
            </section>

            <section className="rounded-[16px] border border-[var(--ui-color-border)] bg-white p-3">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Historial</p>
              <p className="mt-1 text-sm leading-5 text-slate-600">
                Auditoria disponible en backend para cambios de catalogo. La vista de historial detallado queda
                preparada para el endpoint administrativo de auditoria.
              </p>
            </section>
          ...
          <article className="rounded-[16px] border border-dashed border-[var(--ui-color-border)] bg-white p-4">
            ...
            <p className="mt-1 text-sm leading-5 text-slate-600">
              Selecciona una clase para revisar comportamiento POS, productos vinculados y advertencias.
            </p>
          </article>
        )}
      </div>
