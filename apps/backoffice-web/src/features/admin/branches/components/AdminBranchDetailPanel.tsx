...
function formatReadiness(value: AdminBranchDetail["overview"]["readiness"] | AdminBranchListItem["readiness"]) {
  const labels = {
    inactive: "Inactiva",
      ...
      <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[20px] border border-[var(--ui-color-border)] bg-white">
        ...
        <div className="flex min-h-0 flex-1 items-center p-3">
          <div className="rounded-[18px] border border-dashed border-[var(--ui-color-border)] bg-slate-50 p-4 text-sm leading-6 text-slate-600">
            ...
            <p>Selecciona una sucursal para revisar su configuracion, estaciones asociadas y estado operativo.</p>
          </div>
        </div>
      </aside>
    );
    ...
    <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[20px] border border-[var(--ui-color-border)] bg-white">
      <div className="flex min-w-0 shrink-0 items-start justify-between gap-2 border-b border-[var(--ui-color-border)] px-3 py-2.5">
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-slate-500" title={`${overview.code} - ${overview.brandName}`}>
        ...
        {branchDetail ? (
          <>
            ...
            <Section title="Configuracion operativa">
              <div className="grid gap-1.5">
                <InfoRow label="POS" value={branchDetail.operationalConfig.posReady ? "Listo" : "No listo"} />
                <InfoRow
                  label="Inventario"
                  value={branchDetail.operationalConfig.inventoryScopeReady ? "Preparado" : "No listo"}
                />
                <InfoRow
                  label="Produccion"
                  value={branchDetail.operationalConfig.productionScopeReady ? "Preparado" : "No listo"}
                />
                <InfoRow
                  ...
                  <div className="grid gap-1">
                    {branchDetail.workstationsSummary.items.slice(0, 5).map((workstation) => (
                      <div
                        ...
                        key={workstation.id}
                      >
                        <span className="min-w-0 truncate font-semibold text-slate-900" title={workstation.name}>
                          {workstation.name}
                        </span>
              ...
              {branchDetail.userAssignmentsSummary.items.length > 0 ? (
                <div className="grid gap-1.5">
                  ...
                  {branchDetail.userAssignmentsSummary.items.slice(0, 5).map((assignment) => (
                    ...
                    >
                      <p className="truncate font-semibold text-slate-900" title={assignment.userName}>
                        {assignment.userName}
                      </p>
                ...
                <p className="text-sm leading-6 text-slate-600">Esta sucursal no tiene usuarios asignados.</p>
              )}
            </Section>
          </>
        ) : null}
